...
import { PWA_ONBOARDING_KEY } from '../../components/PwaOnboarding';
  ...
  return (
    <aside className="hidden md:flex flex-col w-56 lg:w-64 sticky top-0 h-screen bg-white border-r border-slate-100 shadow-[1px_0_3px_0_rgba(15,23,42,0.04)]"
      ...
      <div className="border-t border-orange-100/60 px-3 py-3 bg-white/60 backdrop-blur-sm">
        ...
        {/* Reset PWA welkom-scherm — handig voor gebruikers die later van rol
            veranderen (bv. bedrijfseigenaar wordt ook huurder bij ander bedrijf). */}
        <button
          onClick={() => {
            try { localStorage.removeItem(PWA_ONBOARDING_KEY); } catch { /* ignore */ }
            // Korte feedback dat het gelukt is. Geen toast hier dependent
            // van een externe lib — alert is voldoende voor deze randactie.
            alert('PWA welkom-scherm is gereset. Sluit en heropen de app om het opnieuw te zien.');
          }}
          data-testid="reset-pwa-onboarding-btn"
          className="w-full flex items-center justify-center gap-2 px-3 py-1.5 mb-1 rounded-xl text-[11px] font-bold text-slate-400 hover:bg-orange-50 hover:text-[#FF5C00] transition-all"
          title="Toon het welkom-scherm opnieuw bij de volgende keer dat de PWA opent"
        >
